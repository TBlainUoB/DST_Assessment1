...
#We can see that in our train data, there are no missing value for the target, which is the label we
#want to predict; Also there are lot more 0's than 1's, because our mean is 0.03.

#%%
missing=dict()
cols = list(trainData0)
...
#%%
print([key  for (key, value) in missing.items() if 0< value <0.05 ])
