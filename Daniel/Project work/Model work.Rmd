...
gini<-function(y_true,y_prob){
    y_true <- as.array(y_true)
    y_true <- y_true[order(y_prob)]
    ntrue <- 0
    ginii <- 0
    delta <- 0
    n <- length(y_true)
    for (i in seq(n,0,-1)){
        ntrue <-ntrue+ y_true[i]
        ginii <-ginii+ y_true[i] * delta
        delta <-delta+ 1 - y_true[i]
    }
    ginii <- 1 - 2 * ginii / (ntrue * (n - ntrue))
    return(ginii)
}
...
normalizedGini(actual,preds.logmod.1)
normalizedGini(actual,preds.logmod.2)
normalizedGini(actual,preds.logmod.3)
normalizedGini(actual,preds.logmod.4)
```
